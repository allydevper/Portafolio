...
const ProjectTable: React.FC<ProjectTableProps> = ({ projects, loading, handleDelete, handleEdit }) => {
    ...
    return (
        <div className="bg-gray-800 dark:bg-gray-800 shadow-xl rounded-lg lg:col-span-2">
                                    ...
                                    projects.map((project, index) => (
                                        <tr key={index} className="hover:bg-gray-700 dark:hover:bg-gray-700 border-b border-gray-600 dark:border-gray-700">
                                            <td className="py-4 px-4">
                                                <div className="inline-flex gap-2">
                                                    <button onClick={() => handleOpenConfirmDelete(project.id)} aria-label="Eliminar proyecto" className="cursor-pointer text-red-500 hover:text-white flex items-center">
                                            ...
                                            <td className="py-4 px-4">
                                                {
                                                    !project.url_demo ? "" :
                                                        <a href={project.url_demo} target='_blank' className="cursor-pointer text-gray-400 hover:text-white flex items-center">
            ...
            <ConfirmDialog
                isOpen={selectedId > 0}
                close={() => setSelectedId(0)}
                title="¿Estás seguro?"
                message={`¿Quieres eliminar el item?`}
                onConfirm={() => selectedId > 0 && handleDelete(selectedId)}
